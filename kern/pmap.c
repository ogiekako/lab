...
mem_init(void)
{
	...
	boot_map_region(kern_pgdir, UPAGES,
			ROUNDUP(npages * sizeof(struct PageInfo), PGSIZE), PADDR(pages), PTE_U);
...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		...
		if (i == 0) {
			...
			ref = 1; // I/O, BIOS ROM 
		} else if (EXTPHYSMEM / PGSIZE <= i && i < EXTPHYSMEM / PGSIZE + 1024) {
			ref = 1; // 4MB kernel region set up in entry.S
		}
		pages[i].pp_ref = ref;
		if (ref == 0) {
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	struct PageInfo *res = pa2page(PTE_ADDR(*pte) | PGOFF(va));
	assert(res->pp_ref > 0);
	assert(res->pp_link == NULL);
	if (pte_store) {
		*pte_store = pte;
	}
	return res;
